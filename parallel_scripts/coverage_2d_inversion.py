...
This script is based on ./coverage_2d.py, but computes all intervals instead
of just the ones based on the max estimated quantile.

See ./dev_notebooks/test_inversion_2d.ipynb for a more methodogical breakdown
of each step.
...
def compute_intervals_i(
    i, data_i, alpha, gamma, eta, M, num_samp,
    method, hp_dict
):
    """
...
if __name__ == "__main__":
    ...
    GAMMA = ALPHA - ETA  # (ALPHA - ETA) / (1 - ETA)
    METHOD = 'rolling'
    ...
    SAVE_FP += f'_alpha{ALPHA}_eta{ETA}_bbcalib_center_roll.npz'
